...
sed "s/SITENAME/$SITE_NAME/g" source/deploy_tools/nginx.template.conf \
...
sed "s/SITENAME/$SITE_NAME/g" source/deploy_tools/gunicorn-systemd.template.service \
	| sudo tee /etc/systemd/system/gunicorn-$SITE_NAME.service
